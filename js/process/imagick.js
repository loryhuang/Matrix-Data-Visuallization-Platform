...
define([
    ...
    var showImage = function () {
                ...
                var publicData = {};      
                publicData.imageUrl =  Const.TEMP_IMG_URL + imagename;  
                ...
                function getGeneItems(x, y, w, h) {
                    ...
                    var grideSize = imageZoom * 2 ; //每个反应值的size
                    
                    if(imageZoom == 1){
                       var resize = 8 ; //imagick切图后放大的倍数
                    }else{
                       var resize = 1 ; 
                    }
                    ...
                    var colStartIndex = Math.round(x / grideSize);
                    var colLength = Math.round(w / grideSize);
                    var rowStartIndex = Math.round(y / grideSize);
                    var rowLength = Math.round(h / grideSize);
                    ...
                    //超出窗口就缩小,否则按实际大小
                    if ((resize * w + $('#sr').width() + 25) > $(window).width()){
                        $('#l').width($(window).width() - $('#sr').width() - 45);
                    }else{
                        $('#l').width(resize * w + 25);
                    }

                    if(($('#t').height() + resize * h + 25) > $(window).height()){
                        $('#img,#r').height($(window).height() - $('#t').height() - 25);
                    }else{
                        $('#img,#r').height(resize * h + 25);
                    }
