...
var IIPMooViewer = new Class({
    ...
    initialize: function(main_id, options) {
        ...
        this.navWinSize = options.navWinSize || 0.15;
    ...
    key: function(e) {
        ...
        //added by huang ,if there is text to input ,disable key events
        var disableKey = $('#mark_title:visible').length;
        if (disableKey) {
            return;
        }
        //
        ...
        switch (e.code) {
            ...
            case 27:
                ...
                break;
            case 70:
                // f fullscreen, but if we have multiple views
                if (!IIPMooViewer.sync) this.toggleFullScreen();
                break;
            default:
                break;
    ...
    createAnnotations: function() {
        ...
        for (var i = 0; i < this.annotations.length; i++) {
            ...
            if (this.wid * (this.annotations[i].x + this.annotations[i].w) > this.view.x && this.wid * this.annotations[i].x < this.view.x + this.view.w && this.hei * (this.annotations[i].y + this.annotations[i].h) > this.view.y && this.hei * this.annotations[i].y < this.view.y + this.view.h
    ...
    reload: function() {
        ...
        if (this.scale) {
            this.updateScale();
            pos = document.id(this.source).getSize().y - document.id(this.source).getElement('div.scale').getSize().y - 10;
